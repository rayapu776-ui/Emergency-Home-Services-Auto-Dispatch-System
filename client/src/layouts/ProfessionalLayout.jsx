    ...
    <div className="min-h-screen bg-[#f6f7f3] text-slate-900 font-sans technician-dashboard selection:bg-emerald-200 flex flex-col">
      ...
      <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md text-slate-900 px-4 md:px-6 py-2.5 flex items-center justify-between border-b border-slate-200/90 shadow-xs">
        ...
        <div className="flex items-center gap-2.5 sm:gap-3">
          ...
          {/* Top Navbar Profile Avatar — Synced with backend users.avatar */}
          <NavLink
            to="/technician/profile"
            title={techProfile?.name ? `${techProfile.name} (View Profile)` : "Professional Profile"}
            className="w-8 h-8 sm:w-9 sm:h-9 rounded-full overflow-hidden border-2 border-emerald-600/40 bg-emerald-50 flex items-center justify-center shrink-0 hover:ring-2 hover:ring-emerald-600 transition-all cursor-pointer shadow-2xs"
          >
            {techProfile?.avatar ? (
              <img
                src={techProfile.avatar}
                alt={techProfile.name || "Professional"}
